...
The inputs to the program consist of both MS Excel files along with user specified parameters that are used by the program to include/exclude certain data from the melt shift analysis. Inputs include the following:
  ...
  * function inputs
    ...
    * RunPANTHER: A Yes or No input that indicates whether or not the PANTHER GO term analysis is conducted by the program workflow.
    * PANTHERpvalue: The decimal that corresponds to the p-value limit specified by PANTHER and which is used by the program to determine which GO terms to report. 
...
* GoodQualityMeltShifts.csv: This file contains the melt shifts that pass all criteria entered by the user
* Melt_vs_Abundance.pdf plot: This is a plot of melt shift vs. abundance for proteins that had good quality melts and met PSM/UP criteria. Red dots are those that are considered to be significant (melt shift p-value, melt shift magnitude) while black dots are those that do not meet these criteria.
...
* PANTHER_BP.svg: This file contains an image of a plot that on the y-axis includes Biological Process GO terms that correspond with the significant proteins in the analysis. On the x-axis are the melt shifts for the proteins. Each dot corresponds to a protein. The color of the dot and the intensity of the blue corresponds with the p-value in PANTHER and the size of the dot corresponds with the p-value for the melt shift. Dots that lie further to the right, are larger in diameter and are a darker shade of blue are more significant.
* PANTHER_CC.svg: This file contains an image of a plot that on the y-axis includes Celluclar Component GO terms that correspond with the significant proteins in the analysis. See The BP description above for meaning of data in the plot.
* PANTHER_MF.svg: This file contains an image of a plot that on the y-axis includes Molecular Function GO terms that correspond with the significant proteins in the analysis. See The BP description above for meaning of data in the plot.
 ...
 * ReportPANTHER: This function creates the PANTHER / melt shfit network diagram
...
## Example code for execution of the function
  ...
  InflectSSP::InflectSSP(Directory,NControl,NCondition,PSM,UP,CurveRsq,PValMelt,MeltLimit,RunSTRING,STRINGScore,RunPANTHER,Species,PANTHERpvalue)  
